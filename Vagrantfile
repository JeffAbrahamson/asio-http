...
Vagrant.configure("2") do |config|
  ...
  Vagrant::Config.run do |config|
    # This requires a "ForwardAgent yes" line for 127.0.0.1 in
    # $HOME/.ssh/config on the host.
  ...
  $script_pre = <<SCRIPT_PRE
    sudo apt-get update
    sudo apt-get install -y  git libboost-all-dev libboost-doc emacs gdb gdb-doc
...
SCRIPT_POST
  ...
  config.vm.provision "shell", privileged: false, inline: $script_post
